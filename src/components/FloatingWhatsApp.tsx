...
export default function FloatingWhatsApp({ idioma = IDIOMA_POR_DEFECTO }: Props) {
  ...
  return (
    ...
    // bottom:20px con 56px de lado. Este se pone a su izquierda (right:88px) y
    // un z-index por debajo del suyo, para quedar al lado y no taparlo. El
    // panel del chat se abre por encima de los dos (bottom:88px).
    //
    // En movil no sale: ahi el contacto lo lleva la barra fija de abajo.
    <div
      className="fixed bottom-5 right-[88px] z-[2147482999] hidden md:flex items-center gap-3"
    >
      {/* Botón Flotante Neón */}
      <a
