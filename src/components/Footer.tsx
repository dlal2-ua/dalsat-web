...
import { EMAIL, urlWhatsApp } from '../data/contacto';
...
// Pie estatico: solo enlaces, sin estado ni efectos. No necesita hidratarse.
//
// Los titulos de columna son <h2> con aspecto de etiqueta: antes eran <h4>
// sin h2 ni h3 por encima, y el lector de pantalla veia la jerarquia rota.
const TITULO_COLUMNA = 'text-xs font-mono font-bold uppercase tracking-wider text-white';
...
export default function Footer({ idioma = IDIOMA_POR_DEFECTO }: FooterProps) {
  ...
  return (
    <footer className="relative bg-navy-900 text-white/70 border-t border-white/10 pt-16 pb-12 overflow-hidden">
      {/* Luz de fondo como gradiente de la propia capa, sin blur. */}
      <div
        className="absolute inset-0 pointer-events-none bg-[radial-gradient(ellipse_at_50%_100%,rgba(20,205,236,0.10),transparent_60%)]"
        aria-hidden="true"
      />
      ...
      <div className="max-w-7xl lg:max-w-[1380px] mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-10 pb-12 border-b border-white/10">
          ...
          <div className="lg:col-span-4 space-y-4">
            ...
            {/* Aqui habia un punto parpadeante con "Sistemas funcionando ahora
                mismo". No estaba conectado a nada: se quito. */}
          </div>

          {/* Columna 2: Navegación (lg:col-span-2) */}
          <div className="lg:col-span-2 space-y-3">
            <h2 className={TITULO_COLUMNA}>
              {t.footer.navegacion}
            </h2>
          ...
          <div className="lg:col-span-3 space-y-3">
            ...
            {/* Cada enlace cae en su propia ficha del catalogo (Services.tsx
                pone id={servicio.id} en cada una). */}
            <ul className="space-y-2 text-xs font-medium">
              <li>
                <a href={ruta(rutaServicio('saas'), idioma)} className="hover:text-cian transition-colors">{t.footer.saas}</a>
              </li>
              <li>
                <a href={ruta(rutaServicio('agentes'), idioma)} className="hover:text-cian transition-colors">{t.footer.agentes}</a>
              </li>
              <li>
                <a href={ruta(rutaServicio('panel'), idioma)} className="hover:text-cian transition-colors">{t.footer.panel}</a>
              </li>
              <li>
              ...
              <li>
                <a href={ruta('/sectores', idioma)} className="hover:text-cian transition-colors">{t.footer.sectores}</a>
              </li>
            </ul>
          </div>
          ...
          <div className="lg:col-span-3 space-y-4">
            ...
            <div className="space-y-2.5">
              <a
                ...
                className="group flex items-center justify-center gap-2.5 bg-terracota hover:bg-terracota-light text-navy-950 font-bold text-xs px-5 py-3 rounded-xl transition-all duration-300 shadow-[0_0_20px_rgba(217,100,44,0.3)] hover:scale-105"
              >
        ...
        <div className="pt-8 space-y-5">
          <nav aria-label={t.footer.legal}>
            <ul className="flex flex-wrap items-center justify-center gap-x-6 gap-y-2 list-none p-0 m-0 text-xs font-medium text-white/70">
              <li>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-3 text-xs font-mono text-white/60">
            <p>{t.footer.derechos}</p>
            <span>{t.footer.ambito}</span>
          </div>
        </div>
